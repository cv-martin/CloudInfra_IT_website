    ...
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <section className="pt-32 pb-14 bg-[#020510] relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none opacity-30"
          style={{ backgroundImage: "linear-gradient(to right,rgba(245,158,11,.05) 1px,transparent 1px),linear-gradient(to bottom,rgba(245,158,11,.05) 1px,transparent 1px)", backgroundSize: "80px 80px" }} />
        <div className="ci-container relative z-10 max-w-2xl">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <span className="block text-[11px] font-bold uppercase tracking-widest text-amber-400 mb-2">Specialty</span>
            <h1 className="text-[clamp(1.9rem,3vw,2.8rem)] font-bold text-white leading-tight">Manufacturing &amp; Industrial Staffing</h1>
            <p className="text-sm text-white/50 mt-4 leading-relaxed">
              Placing Automation, Production, Quality, and Supply Chain professionals across US manufacturing plants — contract, permanent, and temp-to-hire.
            </p>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              <Link href="/job-opportunities" className="inline-flex items-center gap-2 bg-amber-500 text-white text-sm font-semibold px-6 py-3 rounded-full hover:bg-amber-600 active:scale-95 transition-all">
                Browse Manufacturing Roles <ArrowRight className="h-4 w-4" />
              </Link>
        ...
        <section className="bg-white border-b border-gray-100">
          <div className="ci-container py-14">
            <motion.div variants={fade} initial="hidden" whileInView="show" viewport={{ once: true }} className="mb-10">
              <span className="block text-[11px] font-bold uppercase tracking-widest text-amber-500 mb-2">Manufacturing Disciplines</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {disciplines.map((d, i) => { const Icon = d.icon; return (
                <motion.div key={d.label} variants={fade} initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ delay: i * 0.06 }}
                  className="rounded-2xl border border-gray-100 bg-[#F8F9FB] p-6 hover:border-amber-200 hover:shadow-sm transition-all">
                  <div className="w-9 h-9 rounded-xl bg-amber-50 border border-amber-100 flex items-center justify-center mb-4">
                    <Icon className="text-amber-500" style={{ width: 18, height: 18 }} />
                  </div>
        ...
        <section className="bg-gray-950">
          <div className="ci-container py-12">
            <motion.div variants={fade} initial="hidden" whileInView="show" viewport={{ once: true }}
              ...
              <div>
                <h2 className="text-lg font-bold text-white mb-2">Find or fill a manufacturing role.</h2>
                <p className="text-sm text-gray-400 max-w-md">Plant hiring manager or industrial professional — we respond within 24 hours.</p>
              </div>
